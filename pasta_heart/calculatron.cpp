...
int main()
{
	...
	for (int i = 1; i <= cases; i++) // iterate over test cases
	{
		...
		// create 2D dynamic array to hold the distances between planets
		double** distances = new double*[planetCount];
		for (int j = 0; j < planetCount; j++)
			distances[j] = new double[planetCount];
		...
		for (int j = 0; j < queryCount; j++) // do this for every single query
		{
			...
			*/
			...
			portalDiscounts = NULL;
			delete[] visited; // free up memory
			visited = NULL;
		}
		delete[] distances; // free memory
		distances = NULL;
