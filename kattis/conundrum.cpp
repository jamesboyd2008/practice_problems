/*
This program, given a string, outputs the number of characters in the string that do not match up with the pattern: PERPERPERPERPER...

This problem is from https://open.kattis.com/problems/conundrum
...
int main()
{
  string input, per = "PER";
  int total = 0, count = 0;
  cin >> input;

  for (int i = 0; i < input.size(); i++)
  {
    if (input[i] != per[count % 3])
      total++;
    count++;
  }
