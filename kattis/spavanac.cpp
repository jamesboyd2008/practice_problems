/*
This program takes 45 minutes off a 24-hour clock.

from https://open.kattis.com/problems/spavanac
...
int main()
{
  int in_hours, in_minutes, just_minutes, out_hours, out_minutes;
  cin >> in_hours >> in_minutes;

  just_minutes = in_minutes + in_hours * 60;
  just_minutes -= 45;

  // python style modulus in c++ ==> (b + (a%b)) % b
  just_minutes = (24*60 + (just_minutes % (24*60))) % (24*60);

  out_hours = just_minutes / 60;
  out_minutes = just_minutes - out_hours * 60;

  cout << out_hours << " " << out_minutes << endl;
